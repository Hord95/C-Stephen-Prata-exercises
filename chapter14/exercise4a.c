...
void show(struct insurance * res)
{
    ...
    for (int i=0;i<5;i++)
    {
        if((*(res+i)).human.secondname[0]=='\0')
            printf("%s, %s -- %d\n",(res+i)->human.lastname,(res+i)->human.firstname,(res+i)->snils);
        else
            printf("%s, %s %c. -- %d\n",(res+i)->human.lastname,(res+i)->human.firstname,(res+i)->human.secondname[0],(res+i)->snils);
    }
}   